...
export default function CancellationsErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div
      role="alert"
    ...
    >
      ...
      <h2 className="mt-4 text-[17px] font-semibold text-[var(--text)]">
        The cancellation answers could not be loaded
      </h2>
      <p className="mt-2 text-[13px] leading-relaxed text-[var(--text-sec)]">
        The answers failed to arrive, not the cancellations themselves. Nothing has been changed.
        Try again to reload them.
      </p>
      <button
        type="button"
        onClick={onRetry}
