...
export default function EstimateErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-8">
      ...
      <h2 className="mt-4 text-[17px] font-semibold text-[var(--text)]">
        The recompute could not be run
      </h2>
      <p className="mt-2 max-w-[620px] text-[13px] leading-relaxed text-[var(--text-sec)]">
        The stored estimate is untouched. This audit only reads and reports.
      </p>
      <button
        type="button"
        onClick={onRetry}
