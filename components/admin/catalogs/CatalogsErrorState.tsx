...
export default function CatalogsErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div
      role="alert"
    ...
    >
      ...
      <h2 className="mt-4 text-[17px] font-semibold text-[var(--text)]">
        The catalog list could not be loaded
      </h2>
      <p className="mt-2 text-[13px] leading-relaxed text-[var(--text-sec)]">
        The catalogs themselves are fine — nothing has been changed. Try again to reload the list.
      </p>
      <button
        type="button"
        onClick={onRetry}
