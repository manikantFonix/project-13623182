...
export default function InvoicesEmptyState({ onClear }: { onClear: () => void }) {
  return (
    <div className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] px-6 py-12 flex flex-col items-center text-center">
      ...
      <h3 className="mt-4 text-[15px] font-semibold text-[var(--text)]">No invoices match</h3>
      <p className="mt-2 max-w-[440px] text-[13px] leading-relaxed text-[var(--text-sec)]">
        No invoice matches the current search and filters.
      </p>
      <button
        type="button"
        onClick={onClear}
