...
import { VARIANT_LABELS, type RecordConfig, type RecordsPreview } from './data';

interface Option {
  value: RecordsPreview;
  label: string;
}

export default function RecordsStateControl({
  config,
  state,
  onChange,
}: {
  config: RecordConfig;
  state: RecordsPreview;
  onChange: (state: RecordsPreview) => void;
}) {
  ...
  const detailOptions: Option[] = [
    ...config.detailVariants.map((variant) => ({
      value: `detail-${variant}` as RecordsPreview,
      label: VARIANT_LABELS[variant],
    })),
    { value: 'detail-loading', label: 'Loading' },
    { value: 'detail-error', label: 'Error' },
  ];
  ...
  const group = (label: string, options: Option[]) => (
    <div>
      <p className="px-0.5 pb-1.5 text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--muted-text)]">
        {label}
      </p>
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[80] w-[min(360px,calc(100vw-32px))] bg-[var(--surface)] border border-[var(--border)] rounded-[16px]">
      ...
      <div className="p-2.5 flex flex-col gap-3">
        {group('List', listOptions)}
        {group('Record', detailOptions)}
      </div>
