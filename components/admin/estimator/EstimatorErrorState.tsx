...
export default function EstimatorErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div
      role="alert"
    ...
    >
      <span className="w-9 h-9 rounded-full bg-[var(--amber-bg)] flex items-center justify-center text-[var(--alert-strong)]">
        <i className="ri-error-warning-line text-[20px]" aria-hidden="true"></i>
      </span>
      <h2 className="mt-4 text-[17px] font-semibold text-[var(--text)]">
        The estimator settings could not be loaded
      </h2>
      <p className="mt-2 text-[13px] leading-relaxed text-[var(--text-sec)]">
        Estimates continue on the settings already in place. Try again to reload them.
      </p>
      <button
        type="button"
        onClick={onRetry}
