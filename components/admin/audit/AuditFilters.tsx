...
const scopes: { value: AuditScopeFilter; label: string }[] = [
...
export default function AuditFilters({
  ...
  onScope: (value: AuditScopeFilter) => void;
  group: string;
  ...
  period: AuditPeriod;
  onPeriod: (value: AuditPeriod) => void;
  shown: number;
  total: number;
}) {
  return (
    <div className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-3 flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[260px]">
          ...
          <input
            ...
            aria-label="Search the audit trail by subject or reason"
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            role="group"
            aria-labelledby="audit-scope-label"
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-[var(--muted)] pt-3">
        <FilterMenu label="Filter by type" value={group} options={GROUP_OPTIONS} onChange={onGroup} />
        <FilterMenu
          label="Filter by administrator"
          value={actor}
        ...
        <FilterMenu
          ...
          onChange={(value) => onPeriod(value as AuditPeriod)}
        />
