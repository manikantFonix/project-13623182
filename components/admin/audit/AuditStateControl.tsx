...
import type { AuditState } from './data';

const options: { value: AuditState; label: string }[] = [
  { value: 'populated', label: 'Populated' },
  { value: 'acts', label: 'Acts only' },
  { value: 'access', label: 'Record access only' },
  { value: 'noMatch', label: 'Filtered to nothing' },
  { value: 'oneAdmin', label: 'Filtered to one administrator' },
  { value: 'loading', label: 'Loading' },
  { value: 'error', label: 'Error' },
];

export default function AuditStateControl({
  state,
  onChange,
}: {
  state: AuditState;
  onChange: (state: AuditState) => void;
}) {
