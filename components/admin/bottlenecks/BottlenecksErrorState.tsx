...
export default function BottlenecksErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div
      role="alert"
    ...
    >
      ...
      <h2 className="mt-4 text-[17px] font-semibold text-[var(--text)]">
        These indicators could not be derived
      </h2>
      <p className="mt-2 text-[13px] leading-relaxed text-[var(--text-sec)]">
        A slow query underneath is the usual cause. Nothing is wrong with the platform.
      </p>
      <button
        type="button"
        onClick={onRetry}
