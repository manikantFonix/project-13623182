...
export default function RetailerDetailStateControl({
  ...
  state: DetailPreview;
  onChange: (state: DetailPreview) => void;
}) {
  ...
  const group = (label: string, options: { value: DetailPreview; label: string }[]) => (
    <div>
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] w-[min(360px,calc(100vw-32px))] bg-[var(--surface)] border border-[var(--border)] rounded-[16px]">
      ...
      <div className="p-2.5 flex flex-col gap-3">
        {group('Screen', screenOptions)}
        {group('Dialogs', dialogOptions)}
      </div>
