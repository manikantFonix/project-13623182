...
export default function WidgetsErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div
      role="alert"
    ...
    >
      ...
      <h2 className="mt-4 text-[17px] font-semibold text-[var(--text)]">
        The list of installations could not be loaded
      </h2>
      <p className="mt-2 text-[13px] leading-relaxed text-[var(--text-sec)]">
        The list failed to arrive, not the widgets. No installation has changed state. Try again to
        reload the list.
      </p>
      <button
        type="button"
