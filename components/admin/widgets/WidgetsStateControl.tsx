...
const options: { value: WidgetsPreview; label: string }[] = [
  { value: 'populated', label: 'Populated' },
  { value: 'none', label: 'Filtered to nothing' },
  { value: 'suspend', label: 'Suspend modal' },
  { value: 'submitting', label: 'Suspend submitting' },
  { value: 'loading', label: 'Loading' },
  { value: 'error', label: 'Error' },
];

export default function WidgetsStateControl({
  state,
  onChange,
}: {
  state: WidgetsPreview;
  onChange: (state: WidgetsPreview) => void;
}) {
  ...
  if (!open) {
    return (
      <button
        ...
        className={`fixed bottom-6 right-6 z-[60] h-9 px-3 rounded-full bg-[var(--surface)] border border-[var(--border)] flex items-center gap-2 text-[13px] font-medium text-[var(--text)] hover:bg-[var(--muted)] transition-colors duration-150 whitespace-nowrap ${focusRing}`}
      >
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] w-[min(340px,calc(100vw-32px))] bg-[var(--surface)] border border-[var(--border)] rounded-[16px]">
