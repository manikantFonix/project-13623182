...
import type { ProvidersPreview } from './data';

const options: { value: ProvidersPreview; label: string }[] = [
  { value: 'populated', label: 'Configured, healthy' },
  { value: 'never-entered', label: 'Key never entered' },
  { value: 'replacing', label: 'Replacing a key' },
  { value: 'replace-confirm', label: 'Replace confirmation' },
  { value: 'saving', label: 'Saving' },
  { value: 'save-failed', label: 'Save failed' },
  { value: 'events-accepted', label: 'Events all accepted' },
  { value: 'events-failures', label: 'Signature failures' },
  { value: 'events-none', label: 'No events yet' },
  { value: 'loading', label: 'Loading' },
  { value: 'error', label: 'Error' },
];

export default function ProvidersStateControl({
  state,
  onChange,
}: {
  state: ProvidersPreview;
  onChange: (state: ProvidersPreview) => void;
}) {
  ...
  if (!open) {
    return (
      <button
        ...
        className={`fixed bottom-6 right-6 z-[80] h-9 px-3 rounded-full bg-[var(--surface)] border border-[var(--border)] flex items-center gap-2 text-[13px] font-medium text-[var(--text)] hover:bg-[var(--muted)] transition-colors duration-150 whitespace-nowrap ${focusRing}`}
      >
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[80] w-[min(360px,calc(100vw-32px))] bg-[var(--surface)] border border-[var(--border)] rounded-[16px]">
